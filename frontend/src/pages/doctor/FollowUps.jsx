  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1000px', margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <div>
          <h1 style={{ fontSize: '1.5rem', fontWeight: 700, color: 'var(--color-text)', margin: 0 }}>Follow-up Schedule</h1>
          ...
          <tbody>
            ...
            {followUps.map(f => (
              <tr key={f.id} style={{ borderBottom: '1px solid var(--color-surface-alt)' }}>
                ...
                <td style={{ padding: '12px 16px' }}>
                  <button onClick={() => navigate('/doctor/calendar', { state: { prefillPatientId: f.patientId, prefillPatientName: f.patientName } })} style={{ background: 'var(--color-info)', color: 'var(--color-surface)', border: 'none', padding: '5px 10px', borderRadius: '5px', fontSize: '0.75rem', cursor: 'pointer' }}>
                    Book Visit
                  </button>
                </td>
              </tr>
