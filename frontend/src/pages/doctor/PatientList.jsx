  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1200px', margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <h1 style={{ fontSize: '1.5rem', fontWeight: 700, color: 'var(--color-text)', margin: 0 }}>My Patients</h1>
        <span style={{ fontSize: '0.8rem', color: 'var(--color-text-muted)' }}>{filteredAndSorted.length} patients</span>
      </div>
      ...
      <div style={{ display: 'flex', gap: '16px', marginBottom: '20px', flexWrap: 'wrap' }}>
        ...
        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} style={{ padding: '10px', borderRadius: '8px', border: '1px solid var(--color-border)', outline: 'none' }}>
          <option value="ALL">All Status</option>
          <option value="Active">Active</option>
          <option value="Inactive">Inactive</option>
        </select>
        <select value={sortField} onChange={e => setSortField(e.target.value)} style={{ padding: '10px', borderRadius: '8px', border: '1px solid var(--color-border)', outline: 'none' }}>
      ...
      <div style={{ background: 'var(--color-surface)', borderRadius: '12px', border: '1px solid var(--color-border)', overflow: 'hidden' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr 200px', padding: '12px 18px', background: 'var(--color-surface-alt)', borderBottom: '1px solid var(--color-border)', fontWeight: 600, fontSize: '0.85rem', color: 'var(--color-text-muted)' }}>
        ...
        {isLoading ? <div style={{ padding: 40, textAlign: 'center', color: 'var(--color-text-muted)' }}>Loading patients…</div> :
          ...
          filteredAndSorted.map((p, i) => (
            <div key={p.id} onClick={() => onPatientClick ? onPatientClick(p.patientId) : navigate(`/doctor/patients/${p.patientId}`)}
              style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr 200px', alignItems: 'center', padding: '14px 18px', borderBottom: '1px solid var(--color-surface-alt)', cursor: 'pointer' }}
