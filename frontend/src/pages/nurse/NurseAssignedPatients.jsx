  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1000px', margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <div>
          <h1 style={{ fontSize: '1.5rem', fontWeight: 700, color: 'var(--color-text)', margin: 0 }}>Assigned Patients</h1>
          <p style={{ margin: 0, fontSize: '0.8rem', color: 'var(--color-text-muted)' }}>Patients currently assigned to your shift</p>
        </div>
        <span style={{ fontSize: '0.85rem', fontWeight: 600, color: '#0f766e', background: '#ccfbf1', padding: '4px 12px', borderRadius: '6px' }}>Shift: Morning (08:00 - 16:00)</span>
      </div>
      ...
      <div style={{ background: 'var(--color-surface)', borderRadius: '12px', border: '1px solid var(--color-border)', overflow: 'hidden' }}>
        {patients.length === 0 ? (
          ...
          patients.map(p => (
            <div key={p.id} style={{ padding: '16px 20px', borderBottom: '1px solid var(--color-surface-alt)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: '#ccfbf1', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#0f766e', fontWeight: 700 }}>
