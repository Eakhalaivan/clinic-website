...
@RestController
@RequestMapping("/api/analytics/lab")
@RequiredArgsConstructor
public class LabAnalyticsController {

    private final LabAnalyticsService labAnalyticsService;
    private final AnalyticsContextFilter analyticsContextFilter;

    @GetMapping("/dashboard")
    @PreAuthorize("hasAnyRole('ADMIN', 'SUPER_ADMIN', 'SYSTEM_ADMIN', 'BRANCH_ADMIN', 'LAB_TECH')")
    public ResponseEntity<Map<String, Object>> getLabDashboard(
        ...
        return ResponseEntity.ok(labAnalyticsService.getLabDashboard(filter));
    }
}
