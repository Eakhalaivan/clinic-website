...
@RestController
@RequestMapping("/api/analytics/opd")
@RequiredArgsConstructor
public class OpdAnalyticsController {

    private final OpdAnalyticsService opdAnalyticsService;
    private final AnalyticsContextFilter analyticsContextFilter;

    @GetMapping("/dashboard")
    @PreAuthorize("hasAnyRole('ADMIN', 'SUPER_ADMIN', 'SYSTEM_ADMIN', 'BRANCH_ADMIN', 'EXECUTIVE', 'RECEPTIONIST', 'DOCTOR')")
    public ResponseEntity<Map<String, Object>> getOpdDashboard(
        ...
        return ResponseEntity.ok(opdAnalyticsService.getOpdDashboard(filter));
    }
}
