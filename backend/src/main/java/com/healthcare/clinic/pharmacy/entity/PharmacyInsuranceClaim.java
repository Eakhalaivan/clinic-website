...
@Entity(name = "PharmacyInsuranceClaim")
@Table(name = "pharmacy_insurance_claims")
public class PharmacyInsuranceClaim {
    ...
    @OneToMany(mappedBy = "insuranceClaim", cascade = CascadeType.ALL, orphanRemoval = true, fetch = FetchType.LAZY)
    private List<PharmacyInsuranceClaimLineItem> lineItems = new ArrayList<>();
    ...
    public List<PharmacyInsuranceClaimLineItem> getLineItems() { return lineItems; }
    public void setLineItems(List<PharmacyInsuranceClaimLineItem> lineItems) { this.lineItems = lineItems; }
}
